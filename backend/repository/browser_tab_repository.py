...
def create_table():
    conn = sqlite3.connect('browser_tabs.db')
    cur = conn.cursor()
    cur.execute('''
...
def insert_browser_tab(browser_tab):
    conn = sqlite3.connect('browser_tabs.db')
    cur = conn.cursor()
    cur.execute('''
        INSERT INTO browser_tab (created_at, updated_at, status, retry_times, tab_url, file_name, size)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ''', (browser_tab.created_at, browser_tab.updated_at, browser_tab.status, 
...
def get_browser_tab_by_id(tab_id):
    conn = sqlite3.connect('browser_tabs.db')
    cur = conn.cursor()
    cur.execute('SELECT * FROM browser_tab WHERE id = ?', (tab_id,))
    row = cur.fetchone()
    conn.close()
    if row:
...
def update_browser_tab(browser_tab):
    conn = sqlite3.connect('browser_tabs.db')
    cur = conn.cursor()
    cur.execute('''
        UPDATE browser_tab
        ...
        WHERE id = ?
    ''', (datetime.now(), browser_tab.status, browser_tab.retry_times, 
          browser_tab.file_name, browser_tab.size, browser_tab.id))
    conn.commit()
    conn.close()

def delete_browser_tab(tab_id):
    conn = sqlite3.connect('browser_tabs.db')
    cur = conn.cursor()
    cur.execute('DELETE FROM browser_tab WHERE id = ?', (tab_id,))
    conn.commit()
    conn.close()
